...
class ResourceLoader {
	...
	// ===== 懒初始化的全局锚点 =====
	static get __hub() {
		const key = "__TAGSY_RESOURCE_LOADER__";
	...
	// ===== 类定义阶段抢占执行权 =====
	static __init = (() => {
		const hub = ResourceLoader.__hub;

		if (!hub.owner) {
			hub.owner = ResourceLoader;
		}
	...
	// ===== 是否主实例 =====
	static __isOwner() {
		return ResourceLoader.__hub.owner === ResourceLoader;
	}
	...
	static async loadAllResources() {
		...
		// 已加载过直接退出
		if (this.__hub.loaded) {
			return;
		}

		this.__hub.loaded = true;
		...
		const resourceGroups = [
			...
			[
				...
				Url + "Source/Behavior/SystemBehavior.js",
			],
			[
				Url + "Source/Task/FrameworkTask.js",
			],
		...
		ResourceLoader.resourcesAdded =
			await ResourceLoader.loadResourcesByGroups(resourceGroups);
	}
}
